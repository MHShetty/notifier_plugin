  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Scaffold(
          body: Stack(
            children: [
              bgColorTn - (bgColor) => Positioned.fill(child: ColoredBox(color: bgColor)),
              Center(
                child: ColoredBox(
                  color: Colors.white,
                  child: IntrinsicHeight(
                    child: Column(
                      children: [
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ~(tc)=> DropdownButton<bool>(
                                  ...
                                  onChanged: (mode) {
                                    tMode = mode;
                                    bgColorTn(tMode==false?tC.last:tC.first);
                                    tc();
                                  }
                              ),
                            ),
                                  ...
                                  itemBuilder: (c,i){
                                    ...
                                    Center(
                                      child: Dismissible(
                                        ...
                                        confirmDismiss: (d) async => tC.length>2,
                                        onDismissed: (d){
                                          tC.removeAt(i);
                                          if(tMode==false) {
                                            if(i==tC.length-1) bgColorTn(tC.last);
                                          }
                                          else if(i==0) bgColorTn(tC.first);
                                        },
                                        child: ~(_)=>InkWell(
                                          onTap: (){
                                            showDialog(
                                                ...
                                                builder: (c) => SimpleDialog(
                                                    children: [
                                                      ColorPicker(
                                                          ...
                                                          onColorChanged: (c){
                                                            tC[i] = c;
                                                            if(tMode==false) if(i==tC.length-1) bgColorTn(tC.last);
                                                            else if(i==0) bgColorTn(tC.first);
                                                            _();
                                                          }
                                                      ),
                        ...
                        ~(p)=>RaisedButton.icon(
                          icon: Icon(bgColorTn.isPaused==false?Icons.pause:Icons.play_arrow),
                          label: Text(bgColorTn.isPaused==false?"Pause":"Play"),
                          onPressed: (){
                            if(bgColorTn.isPerformingTween){
                              if(bgColorTn.isPaused) bgColorTn.play();
                              else bgColorTn.pause();
                            } else {
                              Future f;
                              switch(tMode) {
                                case true: f = bgColorTn.interpolateR(tC, Duration(seconds: tSec)); break;
                                case false: f = bgColorTn.interpolateR(tC, Duration(seconds: tSec), reverse: true); break;
                                default: f = bgColorTn.circularInterpolationR(tC, Duration(seconds: tSec)); break;
                              }
                              f.then((_)=>p());
                            }
                            p();
